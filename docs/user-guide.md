...
dashboard ⋯ → 配置 → 最顶部 **推送渠道** 两个独立 checkbox：

| 渠道 | 适合场景 |
|---|---|
| **飞书 webhook 推送** | 手机随身 / 不长期开浏览器 / 团队群同步 |
| **浏览器桌面通知** | 不想用飞书 / 隐私敏感 / 公司禁飞书 / 仅本机使用 |

两者可同时开（默认）也可关任一个。**关掉飞书**后 `feishu.send_event` 不发起 HTTP；**关掉浏览器**后前端忽略 push_event。

### 启用浏览器桌面通知 3 步

1. dashboard 顶部出现"启用桌面通知"条带 → 点 **[启用]** → 浏览器弹权限对话框 → 允许。
2. **macOS 必查**（最常被遗忘）：苹果菜单 → 系统设置 → 通知 → 浏览器（Chrome/Safari/...）→ **允许通知** ON + **通知样式**选"横幅"或"提醒"（"无" = 静默吞掉）。
3. **macOS 关掉勿扰 / 专注模式**：屏幕右上控制中心 → "专注"。任何模式亮着 = 通知全吞。这是 90% "已授权但没弹"的真凶。

**验证**：⋯ → 测试推送 → 屏幕应弹一条 macOS 横幅"测试推送 · claude-notify"。无视前/后台都会弹（和飞书行为对齐）。
...
**状态翻转链路**（L43 + L44）：
- 在终端**打字回话**：靠 `UserPromptSubmit` hook（L43）—— 重装 hook 让它生效：`python3 scripts/install-hooks.py`
- **菜单选择 / AskUserQuestion / 权限 y/n** 等结构化交互：hook 收不到，靠 `derive_status` 内"活动证据"规则（L44）—— Claude 调下一个工具触发 Heartbeat 后翻 running，通常 ≤ 30s
- 之前停在"疑似挂起"翻不回的 bug 同一并修了：watcher 报 suspect 后 Claude 继续跑，活动证据会让 status 自动翻 running

> **首次升级到 L44 修法须重启 backend**：`derive_status` 改动在 backend 进程内存，必须 kill + 重启进程才生效（前端代码热加载即可，但后端不会）。命令：`pkill -f 'uvicorn backend.app'` → `nohup python -m uvicorn backend.app:app --host 127.0.0.1 --port 8787 &`
...
- 没有 dashboard tab → 新 tab 正常加载并聚焦该 session

**user_choice**：
- 已有 dashboard tab → 新 tab 弹遮罩两按钮："关闭此 tab"（浏览器多半拒绝自动关，需手动 ⌘W）/ "仍在此 tab 加载"（强制把新 tab 当成另一个 dashboard）
- 适合"想分屏看两个 session"或"不信任 focus_old 自动行为"的场景

**已知物理限制**（浏览器层，跨 OS 一致，无法绕过）：
- **浏览器一定会开新 tab**：用户点链接是用户行为，脚本不可阻止 → focus_old 也存在一个新 tab，但内容是白页
- **脚本不能关用户打开的 tab**：`window.close()` 仅限脚本自己 `window.open()` 出来的窗口 → 新 tab 关闭需用户手动 ⌘W
- **跨浏览器不复用**（Chrome ↔ Safari）：BroadcastChannel 只在同源同浏览器同 profile 间通信

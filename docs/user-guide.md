...
dashboard 卡片底部 trace 行会显示这条决策的 reason：

```
最近：22:50 ✓ menu_detected_bypass_dup · 22:49 ✗ policy_off
```

`menu_detected_bypass_dup` 表示"识别到菜单 prompt，跳过 dup 过滤推送"。完整 reason 字典见第五节。
...
| `notif_idle_reminder_1_at_X.Xmin` | ✓ push | 第 1 次 idle 提醒（默认 15min 阈值后） |
| `notif_idle_reminder_2_at_X.Xmin` | ✓ push | 第 2 次 idle 提醒（默认 45min 阈值后，最后一次） |
| `notif_idle_dup(X.Xmin<Ymin)` | ✗ drop | idle prompt 副本，距 Stop 时间没到下一阈值 |
| `notif_idle_max_reminders(2)` | ✗ drop | 已达 3 次上限，永远不再推 |
| `menu_detected_bypass_dup` | ✓ push | Round 11：Claude 列菜单等用户选（`❯ 1. xxx`），跳过 dup 过滤立即推 |
...
→ Round 11 已加菜单识别：transcript 出现 `❯ 数字.` 或 `Type something` 时立即推（reason `menu_detected_bypass_dup`），dashboard 卡片显示 🔥 紧急徽。若仍没推：检查 trace 行是否被 `sidechain_active` / `session_muted_all` / `quiet_hours` 拦了，或反馈 transcript 末 20 行格式异常。

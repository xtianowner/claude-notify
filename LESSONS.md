...
## L50 — 点 OS Notification 切到了别的 Chrome window（含别的 localhost 服务）的根因 = `new Notification` 的 onclick + `window.focus()` 不能跨 window 切前台（已修，R25）

**修改时间**：2026-05-13 16:24:55

**现象**：用户只开了一个 dashboard tab。OS Notification 弹出后点击，浏览器没把 dashboard 切到前台，而是切到了**另一个 Chrome 窗口**——里面是 `localhost:3000` 之类的别的服务。看起来像"误聚焦到别的 localhost tab"。

**先排除一个常见误解**：BroadcastChannel `claude-notify-tab` 仲裁逻辑（飞书 ↗ 链接路径，L45）是严格同源（`127.0.0.1:8787`）扫描，**别的 localhost 端口收不到也发不出**这个 channel。所以不是"别的服务发了 PONG 抢走焦点"。

**真根因**：`frontend/notify.js` 用 `new Notification(title, opts)` 创建通知，onclick handler 跑在创建通知的 dashboard tab 上下文里。Chrome 对**后台 window 的脚本自我切前台**有限制：
- 点通知时 Chrome 只把通知所在的 window 拉前台；
- `n.onclick` 里调的 `window.focus()` 是 dashboard tab 想拉自己所在的（后台）window 前台 —— Chrome 多半不允许；
- 结果：dashboard tab 在它自己的 window 内被选中了，但那个 window 在背景里。用户看到的前台是最近活跃的 Chrome window，恰好是另一个含别的 localhost 服务的窗口。

**为什么 `window.focus()` 失败**：现代 Chrome 仅在 user gesture 上下文允许跨 window focus。Notification onclick 算是有限度的 user activation，但对**非 SW 注册**的 Notification 这条路径里，`window.focus()` 跨 window 的可靠性很差（实测 macOS Chrome 经常切不动）。

**正确方案**：Service Worker + `registration.showNotification(...)` + `notificationclick` handler 里 `clients.matchAll() + client.focus()`：
- SW 上下文的 `client.focus()` 在 notificationclick 是**允许跨 window 切前台的**（user activation 在该上下文可传递给目标 client）；
- `clients.matchAll({type:'window', includeUncontrolled:true})` 是严格同源扫描，天生不会扫到别的 origin / 别的 localhost 端口；
- 没现成 dashboard client 时 `clients.openWindow(url)` 拉起新 tab。

**实施**（R25）：
- 新建 `frontend/sw.js`：root scope，install skipWaiting + activate claim()，notificationclick 选 visible client 优先，先 postMessage 后 focus
- 改 `frontend/notify.js`：`bindPermBar` 时 `navigator.serviceWorker.register("./sw.js")`；注册成功后 `onPushEvent` 优先 `registration.showNotification`，挂 `data:{sid,ev_type,ts}`；不支持 / 注册失败 fallback 到 `new Notification`
- 改 `frontend/app.js`：main 启动加 `navigator.serviceWorker.addEventListener("message", ...)`，收 NOTIFICATION_CLICK 后走 hash 跳转复用 `onHashChange`（带 loadSessions + openDrawer）
- backend 不动：`frontend/sw.js` 被 `StaticFiles("/", frontend)` 自动 serve 在 `/sw.js`，scope `/` 覆盖整个 origin

**教训**：
1. **`new Notification(...)` 不能可靠把 dashboard window 切前台**。任何"点通知回到 dashboard"的诉求都该走 Service Worker `registration.showNotification + clients.matchAll + client.focus` 这套标准流，别用 `new Notification + onclick + window.focus`。
2. 用户看到"切到了别的 localhost tab"不一定意味着代码搞错了 origin。`window.focus()` 切不动时，用户**视觉感知**的"焦点"其实是 Chrome 最近活跃的 window —— 这是 OS / Chrome 行为，不是 BroadcastChannel 设计缺陷。排查这类问题要先区分"代码层焦点"（哪个 JS tab 在 active）和"用户视觉焦点"（哪个 OS window 在前台）。
3. Service Worker 在 localhost / 127.0.0.1 / HTTPS 才能注册。本项目绑 `127.0.0.1:8787`，天然满足；远程访问场景需 HTTPS 才能用 SW 路径，会自动 fallback 到 `new Notification`（行为退化到 R24 之前）。

---

## L51 — 飞书 ↗ 链接里的 `#s=<sid>` 在 OS URL handler 链路被吞，点击后跳转到 Chrome 最近活跃 tab（已修，R26）

**修改时间**：2026-05-13 16:43:03

**现象**：在 L50 / R25 已经把 OS Notification 路径改成 SW 后，用户再点**飞书消息卡片里的 ↗ 链接**：
- 第 1 次：Chrome 没新开 tab，直接切到了用户当前 `localhost:5173` tab（一个完全不相干的 Vite 项目）
- 第 2 次：Chrome 直接切到了用户的 `github.com` tab
- URL bar 完整 URL = `http://localhost:5173/` 或 `https://github.com/`，**完全不含 dashboard 的 host/port/hash**

**先排除的误区**：claude-notify 代码里 0 处 `5173` 引用，飞书消息硬编码发的就是 `http://127.0.0.1:8787/#s=<sid>` —— 不存在"代码把 URL 改错"的可能。

**真根因**：飞书消息用 `msg_type: "text"` 纯文本格式（`backend/feishu.py` `send_event`），飞书 PC 客户端用正则自动识别文本里的 URL。`#` 字符在 OS URL handler 链路上极不稳定：
- shell 注释符（如果飞书底层用 `sh -c "open <url>"` 这类未引号拼接）
- 部分 link parser 把 `#` 之后视为 fragment 而不传给 `open` 命令
- macOS `open` 命令对带 hash 的 URL 在某些版本/语境下行为不一致

实测结果：**整段 URL 被丢弃**，飞书只完成了"激活默认浏览器"这一步，Chrome 落在它最近活跃的 tab（5173 / github 全凭运气）。看起来像"跳到别的 localhost 服务"，其实是 Chrome 自己的 tab 选择行为。

**修复**（R26）：
- `backend/feishu.py:259`：链接从 `http://127.0.0.1:8787/#s={sid}` 改成 `http://127.0.0.1:8787/?s={sid}`。query string 在 shell / OS URL handler / 各种 link parser 里都是稳定支持的，没有 `#` 那种语义二义性。
- `frontend/app.js` `main()` 开头：把入站 `?s=<sid>` 用 `history.replaceState` 转写为 `#s=<sid>`，复用下面所有 hash 导航 / BroadcastChannel 仲裁 / onHashChange 逻辑。地址栏最终显示 `#s=<sid>` 不显示 `?s=`，前进/后退也不留多余条目。
- OS Notification / SW / 内部 BroadcastChannel 路径**保持 hash 不动**：这些走的是 Chrome 内部 API，不经过 OS URL handler，hash 完全安全。
- 兼容性：已发出的历史飞书消息仍是 `#s=` 格式，前端 hashMatch 仍能命中。新发出的消息一律 `?s=`。

**部署**：backend 跑在 `reload=False` 下，feishu.py 改动需手动重启 backend 才生效；前端硬刷一次即可。

**教训**：
1. **凡是要让"非浏览器原生程序"打开的链接，禁用 hash fragment**。飞书 / 微信 / 邮件客户端 / Slack 等的 URL 识别和打开链路都过 OS shell 或它们自己的 URL parser，对 `#` 处理千差万别。给"外部应用→浏览器"的入口一律用 query string。
2. 排查"点链接跳错地方"时**别先怀疑代码层 origin / 路由**。先确认：(a) URL bar 完整内容是什么；(b) Chrome 是否新开了 tab；(c) 是否每次跳错地方都不同。如果每次跳错的地方都不同，且都是用户最近活跃的 tab —— 100% 是 URL 在打开链路上被吞了，问题在 link parser 或 OS handler，不在你的代码。
3. 飞书纯文本消息的 URL auto-detect 是脆弱的；要可靠的可点击链接，应改用 lark interactive card 的 hyperlink 元素。本项目暂保持 text 格式（卡片改造成本大），但 query string 已足以规避此问题。

---

## L52 — Chrome 不向前扫匹配 tab：前面位置的 dashboard tab 抢不到焦点（已修，R27 osascript 接管）

**修改时间**：2026-05-13 17:10:00

**现象**（L51 / R26 改成 query URL 修了"URL 被吞"之后暴露的新痛点）：
- 3 tab 场景：tab2=dashboard, tab3=非 dashboard。点飞书 ↗ 链接 → Chrome 新开 tab4 加载 dashboard，**不复用前面位置的 tab2**
- 3 tab 场景：tab2=dashboard, tab3=dashboard。点击 → Chrome 复用 tab3（最右的 dashboard），navigate 到新 URL，drawer 自动打开 ✓
- URL bar 显示正确（`http://127.0.0.1:8787/#s=<sid>`）；drawer 自动打开了对应 session ✓

**根因**：Chrome 接收外部应用打开的 URL 时，决定 "复用 tab vs 新开 tab" 的策略是 Chrome 自己定的：似乎只检查**最右匹配 host 的 tab**，不向前扫描。dashboard JS 在 Chrome 选好 tab 之后才有机会运行，**JS 层面没办法干预 Chrome 的 tab 选择**。

**用户真实诉求**："不管 dashboard tab 在 tab 栏什么位置，点链接都精准跳到那个 tab。"

**纯前端方案为什么都做不到**：
- BroadcastChannel + PING_FOCUS 仲裁能让老 dashboard tab 接管 hash + drawer state，但 `window.focus()` 在背景 tab 内调用切不动 Chrome window（Chrome 安全限制）。
- Service Worker `client.focus()` 只在 notificationclick 上下文（user activation 可传递）允许跨 window 切前台；普通 message context 不允许。
- 让新 tab `window.close()` 失败（Chrome 不让脚本静默关用户打开的 tab）。
- 任何让"前面位置的 dashboard tab 抢回焦点"的尝试都会被 Chrome 拦截。

**正解（已实施 R27）**：**backend 用 osascript 直接控制 Chrome**。
1. 飞书 ↗ 链接改成 `http://127.0.0.1:8787/o/{sid}` 走 backend 专用 endpoint
2. backend `/o/{sid}` handler：
   - 调 osascript `tell application "Google Chrome"` 扫所有 window/tab，找 URL 匹配 `127.0.0.1:8787` 的 tab → `set active tab index + set index of w to 1 + activate`
   - 同时 `await hub.broadcast({"type":"open_intent","sid":<sid>})` 让 dashboard WS handler 自动 openDrawer(sid)
   - osascript 成功 → 返回轻量"已切到 dashboard"HTML（含 `window.close()` 尝试，多半被拒）
   - osascript 失败 / 无 dashboard tab → `<meta refresh url=/?s={sid}>` 兜底走 R26 仲裁路径
3. frontend `onWsEnvelope` 新增 `open_intent` case：跳 hash + openDrawer
4. 不走 System Events（教训 L02：Accessibility 权限会让 osascript 静默挂死），直接 Chrome scripting 只需 Chrome Automation 权限（首次弹"Python wants to control Chrome"框）

**链路对比**：
| 路径 | tab 切换控制者 | 限制 |
|---|---|---|
| R25 hash + new Notification | window.focus() 在 dashboard JS | Chrome 不让背景 tab 切前台 |
| R25 hash + Service Worker | client.focus() 在 SW context | 仅 notificationclick 上下文有效 |
| R26 query URL | Chrome 内部 tab 选择 | Chrome 不向前扫匹配 tab |
| **R27 backend osascript** | macOS osascript 直接控制 Chrome | 仅 macOS；需 Chrome Automation 权限 |

**教训**：
1. **浏览器层面 JS 限制不可绕，跳出来用 OS 层 API**。Chrome / Safari 对"脚本跨 window 切前台 / 关 tab / 切 tab"的限制非常严格，任何纯前端方案都会撞墙。能 osascript 控制的就别试图 hack 浏览器内部行为。
2. **backend 不一定只该处理数据**。当用户体验依赖于"OS 层动作"（窗口切换 / 终端 focus / 浏览器 tab 激活），让 backend 调 osascript / xdotool / PowerShell 是合理的，比折腾 JS / browser extension 简单可靠多了（参考 `/api/sessions/{sid}/focus-terminal` 也走同模式）。
3. **跨平台代价**：osascript 仅 macOS。设计时要预备 fallback（这里是 meta refresh 退回 R26 query URL 路径），让非 macOS 用户也能用，只是体验降级。
4. **Chrome Automation 权限是 macOS 一次性弹框，没 Accessibility 那么折腾**。`tell application "Google Chrome"` 本身不需要 Accessibility（不像 `tell application "System Events"`），只需要用户在 macOS Settings → Privacy & Security → Automation 里点"允许"。比 L02 那个坑友好多了。

---

## L53 — R27 osascript 把"飞书链接刚打开的 /o/<sid> 那个 tab"自己当成 dashboard tab 误激活（已修，R28）

**修改时间**：2026-05-14 08:35:00

**现象**：**没有任何 dashboard tab 在跑**时，点飞书 ↗ 链接 → 跳转失败：浏览器只显示一个空荡荡的"✓ 已切到 dashboard"提示页，dashboard 本体根本没加载。

**根因**：R27 osascript 用 `URL of t starts with "http://127.0.0.1:8787/"` 匹配 dashboard tab。流程：
1. 用户点 `http://127.0.0.1:8787/o/<sid>` 链接 → Chrome 没现成 dashboard tab，**新开 tab** 加载这个 URL
2. backend 收到 HTTP GET → 调 osascript 扫 Chrome → 找到了**刚才那个新开的 `/o/<sid>` tab**（URL 也 starts with `http://127.0.0.1:8787/`）
3. osascript 把这个 tab 错当成 dashboard，`activate` 它，返回 OK
4. backend 走"成功路径"返回"已切到 dashboard"提示 HTML
5. 用户那个 tab 加载的就是这个提示页，**没有 meta refresh 兜底**，dashboard 永远不加载

**修复**（R28）：osascript 匹配条件加排除：
```applescript
if (theURL starts with "http://127.0.0.1:8787/") and (not (theURL starts with "http://127.0.0.1:8787/o/")) then
```

`/o/...` 是飞书链接专用 endpoint，发起请求的那个 tab 自身永远不该被当作"已有 dashboard tab"。排除后没现成 dashboard tab 时 osascript 返回 NOT_FOUND → backend 走 fallback HTML（`<meta refresh url=/?s=<sid>>`）→ 那个新 tab 自己 navigate 到 `/?s=<sid>` 加载 dashboard → main() 把 `?s=` 转写为 `#s=` → openDrawer ✓
